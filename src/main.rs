...
/// 4. [`validation::run`] — validate emails
/// 5. [`output::run`] — write results
#[tokio::main]
async fn main() {
    let cli = Cli::parse();
    let is_quiet = cli.format == Format::Gophish && !cli.verbose;
    ...
    let valid_emails = validation::run(&cli, &unique_emails, &wildcard_domains, is_quiet).await;
    
    output::run(&cli, &valid_emails, is_quiet);
}
